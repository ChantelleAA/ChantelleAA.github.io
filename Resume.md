---
layout: page
title: Resume
permalink: /Resume/
---

# Chantelle Amoako-Atta  
**Machine Learning Engineer | Data Scientist | Educator | AI Researcher | AI Consultant**  
[LinkedIn](https://www.linkedin.com/in/chantelleaa) | [GitHub](https://github.com/ChantelleAA) | Email: chantelatta@gmail.com | Website: [link](https://chantelleAA.github.io)

---

## Education

**Lappeenranta University of Technology (LUT), Finland**  
*Double Degree MSc in Data-Centric Engineering* (2023 – 2024) 
- Final Grade: 4/5
- Thesis: *Playing Oware Nam-nam with Deep Q-Networks*

**African Institute for Mathematical Sciences (AIMS), Rwanda**  
*MSc in Mathematical Sciences* (2022 – 2023)  
- Final Grade: Distinction  
- Thesis: *Classifying Cardiac Arrhythmia Using Machine Learning*

**University of Ghana**  
*BA in Mathematics with Economics (First Class)* (2017 – 2021)  
- Final Grade: 3.6 / 4.0 (First Class Honours)
- Dissertation: *Canonical Quantization of Free Φ⁴ Scalar Fields*

---

## Technical Skills

- **Languages/Tools**: Python, MATLAB, R, SQL, CUDA, Git, HTML
- **Libraries**: TensorFlow, PyTorch, Django, Scikit-learn, CuPy, LangChain, Gradio
- **Other**: Microsoft Office, LaTeX, Canva, Adobe Photoshop

---

## Experience

**Cape Wesley Consult** – *Machine Learning Engineer (Remote)* (Nov 2024 – Present)  
- Built GPT-based sentiment models using LangChain for social media analysis  
- Developed pipelines for election data modeling and explainable AI outputs

**Adaire Academy, Ghana** – *Data Science Instructor* (Sep 2024 – Present)  
- Taught data science and ML using EPFL-based curriculum  
- Guided students through real-world projects using R and Tidyverse

**AIMS Ghana** – *Academic Tutor* (Sep 2024 – Present)  
- Delivered tutorials and one-on-one mentorship in mathematics and Python  
- Led the Coding Club and taught GMSP students during residential programs

**University of Ghana** – *Teaching Assistant* (Oct 2021 – Sep 2022)  
- Assisted in teaching algebra, calculus, real analysis, and computational math  
- Supported students with assignments, study plans, and concept review

---

## Selected Projects

**Case Mediator Matching System (Ishango AI Hackathon)**  
- Built a Django-based case scheduling system with NLP and transformer models  
- Streamlined case-type classification and improved matching speed

**Machine Learning on GPU (LUT University)**  
- Created a high-performance kNN classifier with CUDA kernels and CuPy  
- Demonstrated major gains in processing time for classification tasks

**3D Digit Classification Using LeapMotion**  
- Designed a neural network to classify 3D handwritten digits  
- Achieved 99% accuracy on LeapMotion sensor data

---

## Awards & Programs

- AIMS–Imperial Global Fellows Program (2024)  
- LUT University Masters Scholarship (2024)  
- 1st Place – SIAM Hackathon, Boise State University (2023)  
- MasterCard Foundation Scholar (AIMS Rwanda)

---

## Conferences & Engagements

- New York University Abu Dhabi NYUAD Quantum Computing Hackathon (2025)
- MICCAI 2024 – Presented work on brain tumor segmentation  (2024)
- Ghana Data Science Summit, IndabaX Ghana (2024)  
- Mathematical Biology Workshop – AIMS Ghana (2024)

---

*Full CV available upon request.*
